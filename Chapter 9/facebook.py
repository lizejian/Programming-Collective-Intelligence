import requests, md5, webbrowser, time
...
class fbsession(object):
	...
	def sendrequest(self, args):
		...
		data = requests.get(FacebookURL, args).text
		return parseString(data)

	def makehash(self, args):
		hasher = md5.new(''.join([x + '=' + atgs[x] for x in sorted(args.keys())]))
	...
	def getfriends(self):
		doc = self.sendrequest({'method': 'facebook.friends.get', 'session_key': self.session_key,
							'call_id': callid()})
		results = []
	...
	def arefriends(self, idlist1, idlist2):
		id1 = ','.join(idlist1)
		id2 = ','.join(idlist2)
		doc = self.sendrequest({'method': 'facebook.friends.arefriends', 'session_key': self.session_key,
								'callid_id': callid(), 'id1': id1, 'id2': id2})
		results = []
		for n in doc.getElementsByTagName('result_elt'):
			results.append(n.firstChild.nodeValue)
		return results
